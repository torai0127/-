...
const SAFETY_DATA: Record<string, SafetyInfo> = {
  KR: {
    ...
    countryCode: 'KR',
    dangerLevel: 0,
    dangerLevelText: DANGER_LEVELS[0],
  ...
  TH: {
    ...
    countryCode: 'TH',
    dangerLevel: 1,
    dangerLevelText: DANGER_LEVELS[1],
...
export function getCountryCode(countryName: string): string | null {
  ...
  return normalized ? COUNTRY_CODES[normalized] || null : null;
}
...
export function formatSafetyInfo(info: SafetyInfo, includeNewsLinks = false): string {
  const levelEmoji = info.dangerLevel === 0 ? '✅'
    : info.dangerLevel === 1 ? '⚠️'
      : info.dangerLevel === 2 ? '🟠'
        : info.dangerLevel >= 3 ? '🔴' : '❓';

  let message = `🌍 ${info.countryName}の安全情報（外務省ベース）\n\n`;
  message += `${levelEmoji} ${info.dangerLevelText}\n\n`;
  message += `📝 ${info.summary}\n\n`;
  message += `🔗 外務省 海外安全ホームページ\nhttps://www.anzen.mofa.go.jp/\n`;
  message += `📅 参考更新日: ${info.lastUpdated}`;

  if (includeNewsLinks) {
    message += `\n\n📰 最新の情勢・ニュース\n`;
    message += `・NHK WORLD JAPAN\nhttps://www3.nhk.or.jp/nhkworld/\n`;
